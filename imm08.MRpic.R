...
sigImmuneFile="IVW.filter.csv"           #IVW方法过滤的结果文件
outcomeName="diffuse large B-cell lymphoma"     #结局中展示疾病的名字
setwd("E:\\K_课题设计\\MR课题\\免疫\\弥漫大B细胞淋巴瘤diffuse large B-cell lymphoma\\08.MRpic")     #设置工作目录

#读取暴露数据输入文件
rt=read.csv(exposureFile, header=T, sep=",", check.names=F)

#读取IVW方法过滤的结果文件
sigImmune=read.csv(sigImmuneFile, header=T, sep=",", check.names=F)

#对疾病相关的免疫细胞进行循环
for(immuneCell in unique(sigImmune$exposure)){
  #提取这个免疫细胞的暴露数据
  i=gsub("\\%|\\/", "_", immuneCell)
  singleExposureFile=paste0(i, ".exposure.csv")
  exposure_set=rt[rt$exposure==immuneCell,]
